...
(function ($, window, document) {
    ...
    function fnReorder(aiOrder, oS) {
        var indexes = [];
        for (var i = 0, iLen = aiOrder.length; i < iLen; i++)
        {
            indexes[i] = oS.aoColumns[i]._ColReorder_iOrigCol ? oS.aoColumns[i]._ColReorder_iOrigCol : (oS.aoColumns[i]._ColReorder_iOrigCol = i);
        }
        for (var i = 0, iLen = aiOrder.length; i < iLen; i++)
        {
            var currIndex = indexes.indexOf(aiOrder[i]);
            if (i != currIndex)
            {
                fnArraySwitch(indexes, currIndex, i);
                $.fn.dataTableExt.oApi.fnColReorder(oS, currIndex, i);
            }
        }
    }
    ...
    $.fn.dataTableExt.oApi.fnColReorder = function (oSettings, iFrom, iTo)
    {
         ...
         */
        if (oSettings._bInitComplete && oSettings.aoColumns[iFrom].bVisible)
        {
...
//            oSettings.aoColumns[i]._ColReorder_iOrigCol = i;
            ...
            if (oSettings._bInitComplete) {
                $(oSettings.aoColumns[i].nTh).unbind('click.DT');
...
//        }
    ...
    var ColReorder = function (oDTSettings, oOpts)
    {
        ...
        this.s = {
            ...
            /**
             * Allow change visibility functionnality
             *  @property allowChangeVisibility
             *  @type     boolean
             *  @default  true
             */
        ...
        for (var i = 0, iLen = oDTSettings.aoColumns.length; i < iLen; i++)
        {
            if (!oDTSettings.aoColumns[i]._ColReorder_iOrigCol)
                oDTSettings.aoColumns[i]._ColReorder_iOrigCol = i;
        }
        this._fnConstruct();
    ...
    ColReorder.prototype = {
        ...
        "_fnConstruct": function ()
        {
            ...
            if (typeof this.s.init.allowChangeVisibility != 'undefined')
            {
                this.s.allowChangeVisibility = this.s.init.allowChangeVisibility;
            }
...
//                }
            ...
            this.s.dt.oApi._fnCallbackReg(this.s.dt, 'aoStateSaveParams', function (oS, oData) {
                that._fnStateSave.call(that, oS, oData);
            }, "ColReorder_StateSave");
            ...
            var oldState = this.s.dt.oLoadedState;
            if (oldState && oldState.ColReorder)
            {
                if (oldState.ColReorder.order && oldState.ColReorder.order.length === this.s.dt.aoColumns.length) {
                    aiOrder = null;
                }
                if (oldState.ColReorder.widths && oldState.ColReorder.widths.length === this.s.dt.aoColumns.length) {
                    var bDone = false;
                            ...
                            if (!bDone) {
                                ...
                                if ($('div.dataTables_scrollBody') != null) {
                                    if ($('div.dataTables_scrollBody').length > 0) {
                                        $('div.dataTables_scrollHead', that.s.dt.nTableWrapper).find('table').width(oldState.ColReorder.tableWidth);
                                        for (i = 0, iLen = that.s.dt.aoColumns.length; i < iLen; i++) {
                                            $(that.s.dt.aoColumns[i].nTh).width(oldState.ColReorder.widths[i]);
                                            that.s.dt.aoColumns[i].sWidth = $(that.s.dt.aoColumns[i].nTh).innerWidth() + "px";
                                        }

                                        $('div.dataTables_scrollBody', that.s.dt.nTableWrapper).find('table').width($('div.dataTables_scrollHead', that.s.dt.nTableWrapper).find('table').width());
                                        var header = $('div.dataTables_scrollHead', that.s.dt.nTableWrapper).find('thead th');
                                        var body = $('div.dataTables_scrollBody', that.s.dt.nTableWrapper).find('thead th');
                                        for (var i = 0; i < header.length; i++) {
                                            $(body[i]).width($(header[i]).width());
                                        }
                                    }
                                }
                            }
...
//                    that._fnOrderColumns.call(that, resort);
//                }
                fnReorder(aiOrder, this.s.dt);
            }

            //colvisibility
            if (this.s.allowChangeVisibility) {
                this.dom.wrapper = document.createElement('div');
                this.dom.wrapper.className = "btn-group col-visibility-setting";

                var dropdownToggle = $('<div class="dropdown-toggle" data-toggle="dropdown"><i class="material-icons md-dark">more_horiz</i></div>');
                var dropdownMenu = $('<ul class="dropdown-menu dropdown-menu-right"></ul>');
                var menuItemTemp = $('<li><a href="#"></a></li>');

                var that = this;
                dropdownToggle.bind('click', function (event) {
                    dropdownMenu.empty();
                    function toggle(item, visible) {
                        visible ? item.addClass('shown') : item.removeClass('shown');
                    }

                    var oldWiths = [];
                    var tableW = $('div.dataTables_scrollHead', that.s.dt.nTableWrapper).find('table').width();
                    var wraperW = $('div.dataTables_scrollHead', that.s.dt.nTableWrapper).width();
                    var scrollXEnabled = that.s.dt.oInit.sScrollX === "" ? false : true;

                    that.s.dt.aoColumns.forEach(function (column, index)
                    {
                        oldWiths[index] = $(column.nTh).outerWidth()
                        if (index >= that.s.fixed) {
                            var menuitem = menuItemTemp.clone();
                            menuitem.find('a').attr('title', $(column.nTh).text()).text($(column.nTh).text());
                            toggle(menuitem, column.bVisible);
                            menuitem.bind('click', function (e) {
                                var visible = !column.bVisible;
                                toggle(menuitem, visible);
                                that.s.dt.oInstance.fnSetColumnVis(index, visible, false);

                                if (wraperW === tableW) {
                                    //implement later
                                } else if (scrollXEnabled) {
                                    tableW += visible ? oldWiths[index] : -oldWiths[index];
                                    for (var i = 0, iLen = that.s.dt.aoColumns.length; i < iLen; i++) {
                                        $(that.s.dt.aoColumns[i].nTh).outerWidth(oldWiths[i]);
                                    }
                                    $('div.dataTables_scrollHead', that.s.dt.nTableWrapper).find('table').width(tableW);
                ...
                dropdownToggle.appendTo(this.dom.wrapper);
                dropdownMenu.appendTo(this.dom.wrapper);
            }
        },
...
//        },
         ...
         *  @private 
         */
        "_fnStateSave": function (settings, data)
        {
...
//
            ...
            {
                iOrigColumn = settings.aoColumns[i]._ColReorder_iOrigCol;
                /* Column filter */
...
//                oState.abVisCols[ iOrigColumn ] = oSettings.aoColumns[i].bVisible;
                ...
                /* Column reordering */
                data.ColReorder.order.push(iOrigColumn);
                data.ColReorder.widths.push($(this.s.dt.aoColumns[i].nTh).width());
            }
            data.ColReorder.tableWidth = $('div.dataTables_scrollHead', this.s.dt.nTableWrapper).find('table').width();
            ...
            if (this.s.allowResize) {
                $(nTh).bind('mousemove.ColReorder', function (e) {
                    if (that.dom.drag === null && that.dom.resize === null)
                    {
                        ...
                        } else if (i >= that.s.fixed) {
                            $(nThTarget).css({'cursor': 'pointer'});
                        } else {
                            $(nThTarget).css({'cursor': ''});
                        }
                    }
                });
            }
        ...
        "_fnMouseDown": function (e, nTh, i)
        {
            ...
            if ($(nTh).css('cursor') == 'col-resize') {
                ...
                this.s.mouse.bSortable = this.s.dt.aoColumns[i].bSortable;
                ...
                ////////////////////
            } else if (this.s.allowReorder && i >= that.s.fixed) {
        ...
        "_fnMouseMove": function (e, colResized)
        {
            ...
            if (this.dom.resize) {
                ...
                var minW = that.s.dt.oInstance.oApi._fnConvertToWidth($(nTh).css('min-width'));
                if (this.s.mouse.startWidth + moveLength >= minW) {
                    var nThNext = $(nTh).next();
                    ...
                    if (scrollXEnabled && $('div.dataTables_scrollHead', this.s.dt.nTableWrapper).length != 0) {
                        $('div.dataTables_scrollHead', this.s.dt.nTableWrapper).find('table').width(this.table_size + moveLength);
                    }
                }
                ////////////////////////
...
//console.log($('div.dataTables_scrollBody', this.s.dt.nTableWrapper).find('thead th'));


                        var header = $('div.dataTables_scrollHead', this.s.dt.nTableWrapper).find('thead th');
                        var body = $('div.dataTables_scrollBody', this.s.dt.nTableWrapper).find('thead th');
                        for (var i = 0; i < header.length; i++) {
                            $(body[i]).width($(header[i]).width());
                        }
                        $('div.dataTables_scrollBody', this.s.dt.nTableWrapper).find('table').width($('div.dataTables_scrollHead', this.s.dt.nTableWrapper).find('table').width());
                    }
                }
                ////////////////////////
        ...
        "_fnMouseUp": function (e, colResized)
        {
            ...
            if (this.dom.drag !== null)
            {
                ...
                {
//                    this.s.dt.oInstance.fnAdjustColumnSizing();
                    this.s.dt.oInstance.oApi._fnAdjustColumnSizing(this.s.dt);
                }
...
//                var scrollXEnabled;
                ...
                this.s.dt.aoColumns[colResized].bSortable = this.s.mouse.bSortable;

                //Save the new resized column's width
                this.s.dt.aoColumns[colResized].sWidth = $(this.s.mouse.resizeElem).innerWidth() + "px";
                if ($('div.dataTables_scrollHead', this.s.dt.nTableWrapper).find('table').width() < this.table_size) {
                    this.s.dt.oInstance.oApi._fnAdjustColumnSizing(this.s.dt);
                }
...
//                }
    ...
    {
        $.fn.dataTableExt.oApi._fnCallbackReg($.fn.DataTable.models.oSettings, 'aoStateLoadParams', function (oS, oData) {
            if (oData && oData.ColReorder)
            {
                if (oData.ColReorder.order && oData.ColReorder.order.length === oS.aoColumns.length) {
                    fnReorder(oData.ColReorder.order, oS);
                }
            }
        }, "ColReorder_StateLoad");
        $.fn.dataTableExt.aoFeatures.push({
            "fnInit": function (oDTSettings) {
                ...
                return oTable._oPluginColReorder.wrapper();
            },
            "cFeature": "R",
            "sFeature": "ColReorder"
        });
